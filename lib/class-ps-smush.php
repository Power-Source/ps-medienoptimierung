...
//Migration Class
require_once PS_SMUSH_DIR . "lib/class-ps-smush-migrate.php";

//Stats
require_once PS_SMUSH_DIR . "lib/class-ps-smush-db.php";

//Include Resize class
require_once PS_SMUSH_DIR . 'lib/class-ps-smush-resize.php';
...
require_once PS_SMUSH_DIR . 'lib/class-ps-smush-async.php';
...
if ( ! class_exists( 'WpSmush' ) ) {
	...
	class WpSmush {
		 ...
		 *
		 */
		var $migrated_version_key = "ps-smush-migrated-version";
		...
		function __construct() {
			...
			add_action( 'plugins_loaded', array( $this, 'ps_smush_async' ) );

			//Handle the Async optimisation
			add_action( 'wp_async_wp_generate_attachment_metadata', array( $this, 'ps_smush_handle_async' ) );

			//Handle the Async optimisation
			add_action( 'wp_async_wp_save_image_editor_file', array( $this, 'ps_smush_handle_editor_async' ), '', 2 );
		...
		function do_smushit( $file_path = '' ) {
			...
			// Local fork: use a single size limit for all users.
			$max_size = PS_SMUSH_PREMIUM_MAX_BYTES;
		...
		function resize_from_meta_data( $meta, $ID = null ) {
			...
			if ( ! empty( $meta['sizes'] ) ) {
				...
				foreach ( $meta['sizes'] as $size_key => $size_data ) {
					 ...
					 */
					$smush_image = apply_filters( 'ps_smush_media_image', true, $size_key );
					if ( ! $smush_image ) {
						continue;
					}
			 ...
			 * @$size string, Size of image being smushed
			 */
			$smush_full_image = apply_filters( 'ps_smush_media_image', true, 'full' );
		...
		function smush_image( $meta, $ID = null ) {
			...
			if ( ( ( ! empty( $_POST['action'] ) && 'upload-attachment' == $_POST['action'] ) || isset( $_POST['post_id'] ) )
			     ...
			     && defined( 'PS_SMUSH_ASYNC' ) && PS_SMUSH_ASYNC
			) {
				return $meta;
			}
			...
			if ( get_transient( "ps-smush-restore-$ID" ) || get_transient( "smush-in-progress-$ID" ) || get_transient( "ps-smush-restore-$ID" ) ) {
				return $meta;
			}

			/**
			 * Filter: ps_smush_image
			 *
			 ...
			 *
			 */
			if ( ! apply_filters( 'ps_smush_image', true, $ID ) ) {
				return false;
			}

			//Set a transient to avoid multiple request
			set_transient( 'smush-in-progress-' . $ID, true, PS_SMUSH_TIMEOUT );
		...
		function _post( $file_path, $file_size ) {
			...
			if ( empty( $file_path ) || ! file_exists( $file_path ) ) {
				$data['success'] = false;
				$data['message'] = __( 'Bild konnte nicht optimiert werden.', 'ps-medienoptimierung' );

				return $data;
			}

			$tmp_file = $file_path . '.ps-smush-tmp';
			if ( ! @copy( $file_path, $tmp_file ) ) {
				$data['success'] = false;
				$data['message'] = __( 'Temporäre Datei für die Optimierung konnte nicht erstellt werden.', 'ps-medienoptimierung' );

				return $data;
			}

			$optimized = $this->optimize_local_image( $tmp_file, $this->lossy_enabled, $this->keep_exif );

			if ( is_wp_error( $optimized ) ) {
				@unlink( $tmp_file );
				$data['success'] = false;
				$data['message'] = $optimized->get_error_message();

				return $data;
			}

			$before_size = (int) $file_size;
			$after_size  = (int) filesize( $tmp_file );
			$bytes_saved = max( 0, $before_size - $after_size );

			$response_data = new stdClass();
			$response_data->before_size = $before_size;
			$response_data->after_size  = $after_size;
			$response_data->bytes_saved = $bytes_saved;
			$response_data->compression = $before_size > 0 ? round( ( $bytes_saved / $before_size ) * 100, 2 ) : 0;
			$response_data->time        = round( microtime( true ) - $start_time, 4 );
			$response_data->api_version = 0;
			$response_data->lossy       = $this->lossy_enabled ? 1 : 0;
			$response_data->keep_exif   = $this->keep_exif ? 1 : 0;
			$response_data->image       = '';

			if ( $bytes_saved > 0 ) {
				$response_data->image = file_get_contents( $tmp_file );
			}
		...
		/**
		 * Runs local optimization for a single image file.
		 *
		 * @param string $file_path
		 * @param bool   $lossy
		 * @param bool   $keep_exif
		 *
		 * @return bool|WP_Error
		 */
		function optimize_local_image( $file_path, $lossy = false, $keep_exif = false ) {
			$mime = $this->get_image_mime_type( $file_path );

			if ( empty( $mime ) ) {
				return new WP_Error( 'invalid_image', __( 'Dieses Bildformat wird nicht unterstützt.', 'ps-medienoptimierung' ) );
			}

			if ( class_exists( 'Imagick' ) ) {
				$optimized = $this->optimize_with_imagick( $file_path, $mime, $lossy, $keep_exif );
				if ( ! is_wp_error( $optimized ) ) {
					return $optimized;
				}
			}

			return $this->optimize_with_gd( $file_path, $mime, $lossy );
		}

		/**
		 * Returns mime type for image file.
		 *
		 * @param string $file_path
		 *
		 * @return string|bool
		...
		/**
		 * Optimize image via Imagick when available.
		 *
		 * @param string $file_path
		 * @param string $mime
		 * @param bool   $lossy
		 ...
		 * @return bool|WP_Error
		 */
		function optimize_with_imagick( $file_path, $mime, $lossy = false, $keep_exif = false ) {
			try {
				$imagick = new Imagick();
				...
				switch ( $mime ) {
					case 'image/jpeg':
						...
						$imagick->setImageCompressionQuality( $lossy ? 82 : 92 );
						$imagick->setInterlaceScheme( Imagick::INTERLACE_PLANE );
						break;
					case 'image/png':
						$imagick->setImageCompression( Imagick::COMPRESSION_ZIP );
						$imagick->setImageCompressionQuality( $lossy ? 82 : 95 );
						break;
					case 'image/gif':
					...
					default:
						...
						return new WP_Error( 'unsupported_image', __( 'Dieses Bildformat wird nicht unterstützt.', 'ps-medienoptimierung' ) );
				}
		...
		/**
		 * Optimize image via GD fallback.
		 *
		 * @param string $file_path
		 * @param string $mime
		 * @param bool   $lossy
		 ...
		 * @return bool|WP_Error
		 */
		function optimize_with_gd( $file_path, $mime, $lossy = false ) {
			if ( ! function_exists( 'gd_info' ) ) {
				return new WP_Error( 'gd_missing', __( 'Kein unterstützter lokaler Bildoptimierer verfügbar (Imagick/GD).', 'ps-medienoptimierung' ) );
			}

			switch ( $mime ) {
				case 'image/jpeg':
					if ( ! function_exists( 'imagecreatefromjpeg' ) || ! function_exists( 'imagejpeg' ) ) {
						return new WP_Error( 'gd_jpeg_missing', __( 'JPEG-Optimierung ist in GD nicht verfügbar.', 'ps-medienoptimierung' ) );
					}
					$image = @imagecreatefromjpeg( $file_path );
					if ( ! $image ) {
						return new WP_Error( 'gd_read_failed', __( 'Bild konnte für die Optimierung nicht gelesen werden.', 'ps-medienoptimierung' ) );
					}
					$ok = imagejpeg( $image, $file_path, $lossy ? 82 : 92 );
					imagedestroy( $image );

					return $ok ? true : new WP_Error( 'gd_write_failed', __( 'Optimiertes Bild konnte nicht gespeichert werden.', 'ps-medienoptimierung' ) );
				case 'image/png':
					if ( ! function_exists( 'imagecreatefrompng' ) || ! function_exists( 'imagepng' ) ) {
						return new WP_Error( 'gd_png_missing', __( 'PNG-Optimierung ist in GD nicht verfügbar.', 'ps-medienoptimierung' ) );
					}
					...
					$compression = $lossy ? 9 : 6;
					$ok          = imagepng( $image, $file_path, $compression );
					imagedestroy( $image );

					return $ok ? true : new WP_Error( 'gd_write_failed', __( 'Optimiertes Bild konnte nicht gespeichert werden.', 'ps-medienoptimierung' ) );
				case 'image/gif':
					if ( ! function_exists( 'imagecreatefromgif' ) || ! function_exists( 'imagegif' ) ) {
						return new WP_Error( 'gd_gif_missing', __( 'GIF-Optimierung ist in GD nicht verfügbar.', 'ps-medienoptimierung' ) );
					}
					$image = @imagecreatefromgif( $file_path );
					if ( ! $image ) {
						return new WP_Error( 'gd_read_failed', __( 'Bild konnte für die Optimierung nicht gelesen werden.', 'ps-medienoptimierung' ) );
					}
					$ok = imagegif( $image, $file_path );
					imagedestroy( $image );
		...
		function columns( $defaults ) {
			$defaults['smushit'] = 'PS Medienoptimierung';
		...
		function validate_install() {
			...
			// Local fork: always unlock full feature set and skip remote membership checks.
			$this->is_pro = true;
		...
		function set_status( $id, $echo = true, $text_only = false, $wrapper = true ) {
			...
			$ps_smush_data      = get_post_meta( $id, $this->smushed_meta_key, true );
			$wp_resize_savings  = get_post_meta( $id, PS_SMUSH_PREFIX . 'resize_savings', true );
			$conversion_savings = get_post_meta( $id, PS_SMUSH_PREFIX . 'pngjpg_savings', true );

			$combined_stats = $this->combined_stats( $ps_smush_data, $wp_resize_savings );
			...
			if ( ! empty( $ps_smush_data ) ) {

				$image_count    = count( $ps_smush_data['sizes'] );
				...
				if ( empty( $wp_resize_savings['bytes'] ) && isset( $ps_smush_data['stats']['size_before'] ) && $ps_smush_data['stats']['size_before'] == 0 && ! empty( $ps_smush_data['sizes'] ) ) {
					...
					if ( $bytes == 0 || $percent == 0 ) {
						...
						if ( $size > 0 ) {
							...
							$image_size = sprintf( __( "<br /> Image Size: %s", "ps-medienoptimierung" ), $size );
				...
				if ( ! empty( $ps_smush_data ) && ! empty( $ps_smush_data['stats'] ) ) {
					$lossy    = ! empty( $ps_smush_data['stats']['lossy'] ) ? $ps_smush_data['stats']['lossy'] : '';
			...
			$text = $this->column_html( $id, $status_txt, $button_txt, $show_button, $ps_smush_data, $echo, $wrapper );
			if ( ! $echo ) {
				return $text;
			}
		}
		...
		function column_html( $id, $status_txt = "", $button_txt = "", $show_button = true, $smushed = false, $echo = true, $wrapper = true ) {
			...
			if ( ! $echo ) {
				$button_class = $wrapper || ! empty( $mode_class ) ? 'button button-primary ps-smush-send' : 'button ps-smush-send';
				$html .= '
				...
				$html .= '<button class="button ps-smush-send' . $mode_class . '" data-id="' . $id . '">
                    <span>' . $button_txt . '</span>
				</button>';
		...
		function migrate() {
			...
			$q       = $wpdb->prepare( "SELECT * FROM `" . $wpdb->postmeta . "` WHERE `meta_key`=%s AND `meta_value` LIKE %s ", "_wp_attachment_metadata", "%ps_smushit%" );
		...
		function smush_retina_image( $id, $retina_file, $image_size ) {
			 ...
			 * @$size string, Size of image being smushed
			 */
			$smush_image = apply_filters( 'ps_smush_media_image', true, $image_size );
			if ( ! $smush_image ) {
				return;
			}
		...
		function get_skipped_images( $image_id, $size_stats, $attachment_metadata ) {
			...
			if ( ! array_key_exists( 'full', $size_stats ) ) {
				...
				if ( ! $this->validate_install() ) {
					...
					if ( ! empty( $file_size ) && ( $file_size / PS_SMUSH_MAX_BYTES ) > 1 ) {
						$skipped[] = array(
							'size'   => 'full',
							'reason' => 'size_limit'
						);
			...
			if ( is_array( $media_size ) ) {
				foreach ( $media_size as $size ) {
					if ( array_key_exists( $size, $attachment_metadata['sizes'] ) && ! array_key_exists( $size, $size_stats ) && ! empty( $size['file'] ) ) {
						...
						if ( ! empty( $image_size ) && ( $image_size / PS_SMUSH_MAX_BYTES ) > 1 ) {
							$skipped[] = array(
								'size'   => 'full',
								'reason' => 'size_limit'
							);
		 ...
		 * @return string
		 */
		function get_detailed_stats( $image_id, $ps_smush_data, $attachment_metadata ) {
			...
			$stats      = '<div id="smush-stats-' . $image_id . '" class="smush-stats-wrapper hidden">
				<table class="ps-smush-stats-holder">
					...
					<tbody>';
			$size_stats = $ps_smush_data['sizes'];
		...
		function load_nextgen() {
			...
			require_once( PS_SMUSH_DIR . '/lib/class-ps-smush-nextgen.php' );
			// Do not continue if integration not enabled or not a pro user.
			if ( ! $opt_nextgen_val || ! $this->validate_install() ) {
				return;
			}
			require_once( PS_SMUSH_DIR . '/lib/nextgen-integration/class-ps-smush-nextgen-admin.php' );
			require_once( PS_SMUSH_DIR . '/lib/nextgen-integration/class-ps-smush-nextgen-stats.php' );
			require_once( PS_SMUSH_DIR . '/lib/nextgen-integration/class-ps-smush-nextgen-bulk.php' );
		...
		function load_s3() {
			...
			//Include Smush Async class
			require_once PS_SMUSH_DIR . 'lib/class-ps-smush-s3.php';
		}
		...
		function show_restore_option( $image_id, $attachment_data ) {
			...
			$pngjpg_savings = get_post_meta( $image_id, PS_SMUSH_PREFIX . 'pngjpg_savings', true );
			if ( ! empty( $pngjpg_savings ) ) {

				//Get the original File path and check if it exists
				$backup = get_post_meta( $image_id, PS_SMUSH_PREFIX . 'original_file', true );
		...
		function get_restore_link( $image_id, $type = 'wp' ) {
			...
			$class = 'ps-smush-action ps-smush-title';
			$class .= 'wp' == $type ? ' ps-smush-restore' : ' ps-smush-nextgen-restore';
		...
		function progress_bar() {
			return '<div class="ps-smush-progress animate hidden"><span></span></div>';
		}
		...
		function get_resmsuh_link( $image_id, $type = 'wp' ) {
			...
			$class = 'ps-smush-action ps-smush-title';
			$class .= 'wp' == $type ? ' ps-smush-resmush' : ' ps-smush-nextgen-resmush';
		...
		function delete_images( $image_id ) {
			...
			if ( $resmush_list = get_option( 'ps-smush-resmush-list' ) ) {
				global $wpsmushit_admin;
				$wpsmushit_admin->update_resmush_list( $image_id, 'ps-smush-resmush-list' );
			}
		...
		function show_resmush( $id = '', $ps_smush_data = array() ) {
			...
			if ( $this->smush_original ) {
				...
				if ( ! empty( $ps_smush_data ) && empty( $ps_smush_data['sizes']['full'] ) ) {
					return true;
				}
			}
			...
			if ( ! $this->keep_exif ) {
				...
				if ( isset( $ps_smush_data['stats']['keep_exif'] ) && $ps_smush_data['stats']['keep_exif'] == 1 ) {
					return true;
				}
			}
		 ...
		 * @return bool
		 */
		function ps_smush_redirect( $plugin ) {

			global $wpsmush_db;

			//Run for only our plugin
			if ( $plugin != PS_SMUSH_BASENAME ) {
				return false;
			}
			...
			add_site_option( 'ps-smush-skip-redirect', true );
		...
		function ps_smush_async() {
			...
			if ( defined( 'PS_SMUSH_ASYNC' ) && ! PS_SMUSH_ASYNC ) {
				return;
			}
		 ...
		 */
		function ps_smush_handle_async( $id ) {

			//If we don't have image id, or the smush is already in progress for the image, return
			if ( empty( $id ) || get_transient( 'smush-in-progress-' . $id ) || get_transient( "ps-smush-restore-$id" )  ) {
				return;
			}
		 ...
		 * @param $id Attachment ID
		 */
		function ps_smush_handle_editor_async( $id, $post_data ) {
		...
		function add_api_message( $api_message = array() ) {
			...
			$o_api_message = get_site_option(PS_SMUSH_PREFIX . 'api_message', array() );
			...
			update_site_option( PS_SMUSH_PREFIX . 'api_message', $message );
		}
	}
...
require_once( PS_SMUSH_DIR . 'lib/class-ps-smush-admin.php' );

//Include Directory Smush
require_once PS_SMUSH_DIR . 'lib/class-ps-smush-dir.php';